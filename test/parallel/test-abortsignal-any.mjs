...
describe(
  ...
  () => {
    ...
    it('should throw when input contains non-signal values', () => {
      assert.throws(() => AbortSignal.any([AbortSignal.abort(), undefined]), {
    ...
    it('returns an aborted signal with the reason of first aborted signal input', () => {
      const signal = AbortSignal.any([
        AbortSignal.abort('some reason'),
        AbortSignal.abort('another reason'),
      ]);
      assert.strictEqual(signal.aborted, true);
      assert.strictEqual(signal.reason, 'some reason');
      signal.addEventListener('abort', common.mustNotCall());
    ...
    it('handles deeply aborted signals', async () => {
      ...
      const composedSignal2 = AbortSignal.any([
        composedSignal1,
    ...
    it('executes abort handlers in correct order', () => {
      ...
      signals.forEach((signal, i) =>
        signal.addEventListener('abort', () => (result += i))
      );
      controller.abort();
...
it('should handle nested AbortSignal.any() calls correctly with already aborted signals', async () => {
  await new Promise((resolve, reject) => {
    ...
    controller.signal.addEventListener('abort', () => {
      try {
        const resultSignal = AbortSignal.any([signal1]);
        assert.strictEqual(
          resultSignal.aborted,
          true,
          'Resulting signal should be aborted'
        );
        assert.strictEqual(
          resultSignal.reason,
          signal1.reason,
          'Abort reason should match the initial aborted signal'
        );
        resolve();
      } catch (error) {
        reject(
          new Error(
            `No error should be thrown, got '${error.name}' '${error.code}'`
          )
        );
      }
    });
